...
#    logo = models.FileField(upload_to='uploads/%Y-%m-%d')
    ...
    def summary(self):
        ...
        for meal in Meal.objects.filter(event=self):
    ...
    def send_notification(self, recipient, action, objects):
        ...
        for gu in GroupUser.objects.filter(group=recipient.group,admin=True):
            ...
            try:
                ...
                gu.user.email_user(subject, msg, 'webmaster@feedtheartists.net')
            except Exception, err:
                sys.stderr.write("[ERROR] Cannot send email to user %s : error : %s " % (gu.user.username, str(err)))
